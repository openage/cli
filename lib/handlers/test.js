...
export const run = async (params) => {
    logger('handlers.test').verbose('execute')
    ...
    for (const key of extractKeys('prompt', raw)) {
        injectables.prompts[key] = await input.get(key)
    }
    ...
    const spec = template.formatter(raw).inject(injectables)
    ...
    logger('handlers.test').verbose('response', response)
