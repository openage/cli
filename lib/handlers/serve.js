...
import { handleFileApi } from '../../web/apis/file.js'
import { handleDirApi } from '../../web/apis/dir.js'
import { handleGitDiffApi } from '../../web/apis/git-diff.js'
import { handleAuthApi } from '../../web/apis/auth.js'
import { handleLogoutApi } from '../../web/apis/logout.js'
...
const ALLOWED_COMMANDS = new Set(['pull', 'push', 'script', 'serve', 'config', 'context', 'test'])
...
const getContextSummary = () => {
    ...
    const now = Date.now()
    const pinnedFiles = settings.get('ux.pinned.files')

    return {
        env: process.env.NODE_ENV || 'prod',
        web: settings.get('pages.web') || 'localhost',
        pinned: Array.isArray(pinnedFiles) ? pinnedFiles : [],
        ...
        settings: settings.get(null, { decrypt: false }) || {}
    }
}
...
const applyGitAction = async (cwd, action, message, filePath = '') => {
    switch (action) {
        case 'stage':
        ...
        case 'commit': {
            ...
            const targetArgs = filePath ? ['--', filePath] : []
            const stageRes = await runProcess('git', ['add', '-A', ...targetArgs], cwd)
            if (!stageRes.ok) {
                return stageRes
            }
            const commitArgs = ['commit', '-m', String(message).trim()]
            if (filePath) commitArgs.push('--only', '--', filePath)
            return runProcess('git', commitArgs, cwd)
        }
        default:
...
<body>
  <div class="app">
    <aside class="sidebar">
      <div class="brand"><img src="https://raw.githubusercontent.com/openage/openage.github.io/master/logo.png" alt="OA CLI"/><div><strong>OA CLI</strong><br /><a href="${contextSummary.web.startsWith('http') ? contextSummary.web : `https://${contextSummary.web}`}/?session-token=${escapeHtml(contextSummary.session.token)}" target="_blank" class="brand-host" title="Open ${escapeHtml(contextSummary.web)} in new tab">${escapeHtml(contextSummary.web)}</a></div></div>
      <div class="nav">
        <h3>Directory</h3>
...
export const execute = async (options) => {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (handleGitActionApi({ req, res, gitFolder, applyGitAction, getGitSummary, withinFolder, file, toPosix })) return
        ...
        if (handleAuthApi({ req, res, auth, getContextSummary, logger })) return
        if (await handleLogoutApi({ req, res })) return
        if (handleContextApi({ req, res, context, getContextSummary, logger })) return
        ...
        if (handleFileApi({ req, res, configFolder, resolveApiDirectoryTarget, withinFolder, file, toPosix, logger })) return
        if (handleDirApi({
            req,
            res,
            port,
        ...
        if (requestPathname === '/CHANGELOG.md') {
            filePath = path.join(cliRootFolder, 'CHANGELOG.md')
        } else if (!requestPathname || requestPathname === '/' || requestPathname === '/index.html') {
