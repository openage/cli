...
const _loginByWeb = async (activateUrl) => {
    ...
    activateUrl = `${activateUrl}${session.id}`
...
export const login = async () => {
    ...
    let activateUrl = settings.get('auth.session.activate.url')

    if (!activateUrl && application) {
        if (application.nav && application.nav['sessions.activate']) {
            const nav = application.nav['sessions.activate']
            activateUrl = typeof nav === 'string' ? nav : (nav.url || nav.src)
        }

        if (!activateUrl && application.navs) {
            const nav = application.navs.find(n => n.code === 'sessions.activate')
            if (nav) {
                activateUrl = nav.url || nav.src
            }
        }
    }
