...
export const discover = () => {
    ...
    logger('services.specs').silly('count', files.length)
...
export const has = (code) => {
    return Boolean(_specs.get(code))
}

export const register = (spec) => {
    if (!spec || !spec.code) {
    ...
    _specs.set(spec.code, item)
    return item
}

export const status = (code, status) => {
    let spec = _specs.get(code)
    if (!spec) {
        spec = {
            code,
            name: code
